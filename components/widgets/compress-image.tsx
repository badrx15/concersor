...
export function CompressImageWidget() {
  ...
  const convert = async () => {
    ...
    try {
      ...
      setResult({ url, name: replaceExt(file.name, 'jpg'), size: blob.size, original: file.size });
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      <FileDrop accept="image/jpeg,image/png,image/webp" onFiles={(f) => { setFile(f[0]); setResult(null); }} label="Selecciona una imagen JPG o PNG" />
      <div>
        <label className="block text-sm font-semibold text-slate-200 mb-1.5">Nivel de compresión (1-100, menor = más compresión)</label>
        <input type="range" min={1} max={100} value={quality} onChange={(e) => setQuality(+e.target.value)} className="w-full accent-brand-500" />
        <span className="text-sm text-slate-400">Calidad: {quality}</span>
      </div>
      <button onClick={convert} disabled={busy || !file} className="w-full px-6 py-4 rounded-xl font-bold bg-gradient-to-r from-brand-600 to-pink-500 text-white disabled:opacity-50 transition">
        {busy ? <span className="loading-dots"><span></span><span></span><span></span></span> : '🗜️ Comprimir imagen'}
      </button>
